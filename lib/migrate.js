...
function migrateRenamedCategories(isLocal) {
  const configPath = isLocal ? PROJECT_CONFIG_PATH : GLOBAL_CONFIG_PATH;
  const commandsDir = isLocal ? PROJECT_COMMANDS_DIR : GLOBAL_COMMANDS_DIR;
  ...
  for (const [oldName, newName] of Object.entries(RENAMED_CATEGORIES)) {
    const oldSkills = config.categories[oldName];
    if (!oldSkills) continue;

    const newSkills = config.categories[newName] || [];
    config.categories[newName] = [...new Set([...newSkills, ...oldSkills])];
    delete config.categories[oldName];
    changed = true;

    try { fs.unlinkSync(path.join(commandsDir, `st-${oldName}.md`)); } catch {}
  }
...
function runMigration(isLocal) {
  ...
  if (migrateRenamedCategories(isLocal)) {
    console.log('  ✓ Renamed category slugs in config');
    migrated = true;
  }
